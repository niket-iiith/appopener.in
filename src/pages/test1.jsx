...
const EditLinkForm = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(process.env.REACT_APP_API_URL + "createOpenURL", {
        method: "POST",
  ...
  return (
    <div
      className={`w-full max-w-2xl mx-auto space-y-6 px-4 py-6 rounded-md shadow-md ${
    ...
    >
      ...
      {/* Short ID */}
      <div>
        <label className="text-sm text-gray-600 font-medium">Short ID</label>
        <div className="flex flex-col sm:flex-row gap-2 mt-1">
          <input
            type="text"
            value={currentShortId}
            onChange={(e) => {
              setCurrentShortId(e.target.value);
              setIdVerificationResult(null);
            }}
            className={`w-full px-3 py-2 rounded border focus:outline-none text-sm border-gray-300 focus:border-blue-500`}
            placeholder="Enter domain-safe short ID"
          />
          <button
            type="button"
            onClick={verifyShortId}
            disabled={verifyingId}
            className="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700 disabled:opacity-50 w-full sm:w-auto"
          >
        ...
        {idVerificationResult && (
          <p
            className={`mt-1 text-sm ${
              idVerificationResult.status === "success"
                ? "text-green-600"
                : idVerificationResult.status === "error"
                ? "text-red-600"
                : "text-blue-500"
            }`}
          >
            {idVerificationResult.message}
          </p>
        )}

        {/* Helpful hint */}
        <p className="mt-1 text-xs text-gray-500">
          Use only letters, numbers, and hyphens. Cannot start/end with hyphen
          or contain consecutive hyphens.{" "}
          {VideoId ? "Video ID: " + VideoId : "Sujal"}
        </p>
      </div>
      ...
      {/* <div>
        ...
        <input
          ...
          className="w-full px-3 py-2 rounded border border-gray-300 focus:outline-none focus:border-blue-500 text-sm mt-1"
        />
      </div> */}
      ...
      {/* Thumbnail Section */}
      <div>
        <label className="text-sm text-gray-600 font-medium">Thumbnail</label>

        <div className="flex flex-wrap gap-4 mb-2 mt-1 text-sm">
        ...
        {thumbnailSource === "url" ? (
          <input
            ...
            className="w-full px-3 py-2 rounded border border-gray-300 focus:outline-none focus:border-blue-500 text-sm"
          />
        ) : (
          <>
            <input
              ...
              className="w-full text-sm mt-1 text-gray-800 file:mr-4 file:py-1 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700"
            />
            {selectedFile && (
        ...
        {imagePreview && (
          <img
            ...
            className="mt-3 w-32 h-32 object-cover rounded border border-gray-300"
          />
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mt-2">
        <button
          ...
          className="bg-blue-600 text-white font-semibold px-4 py-2 rounded hover:bg-blue-700 transition disabled:opacity-50 w-full sm:w-auto"
        >
          {uploadingImage
            ? "Uploading..."
            : loading
        ...
        <button
          ...
          onClick={onCancel}
          disabled={loading || uploadingImage}
          className="bg-gray-500 text-white font-semibold px-4 py-2 rounded hover:bg-gray-600 transition disabled:opacity-50 w-full sm:w-auto"
        >
          Cancel
        </button>
      </div>
